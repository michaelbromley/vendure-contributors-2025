...
async function main() {
  ...
  const contributors: ContributorData[] = JSON.parse(fs.readFileSync(contributorsPath, 'utf-8'));
  const issueContributors: IssueContributor[] = JSON.parse(fs.readFileSync(issuesPath, 'utf-8'));

  // Build a map of all unique usernames with their commit/issue counts
  const userMap = new Map<string, { commitCount: number; issueCount: number; avatar_url: string; html_url: string }>();
